...
class BaseOCRV20:
    ...
    def read_pytorch_weights(self, weights_path,device='cpu'):
    ...
    def get_out_channels(self, weights):
        out_channels = list(weights.values())[-1].cpu().detach().numpy().shape[0]
        return out_channels
    ...
    def load_pytorch_weights(self, weights_path, device='cpu'):
        self.net.load_state_dict(torch.load(weights_path, map_location=device))
        print('model is loaded: {},device:{}'.format(weights_path, device))
